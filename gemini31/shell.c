...
void execute_command(char* command) {
    ...
    if (str_compare(command, "help")) {
        ...
        fat16_cat_test();
        // ==========================================
    // УНИВЕРСАЛЬНЫЙ ЗАПУСК ПРОГРАММ (run <filename>)
    // ==========================================
    } else if (str_starts_with(command, "run ")) {
        // Извлекаем имя файла (пропускаем "run ")
        char* target = command + 4;
        
        // FAT16 требует формат "ИМЯ     РАСШИРЕНИЕ" (строго 11 символов)
        // Для простоты мы жестко захардкодим два наших тестовых файла:
        if (str_compare(target, "app.bin")) {
            fat16_run("APP     BIN"); 
        } 
        else if (str_compare(target, "sysinfo.bin")) {
            // Перед запуском ОКОННОГО приложения, мы обязаны включить GUI!
            gui_start_desktop(); 
            fat16_run("SYSINFO BIN"); 
        } 
        else {
            draw_string("Unknown program format.\n", VGA_LRED);
        }

        // Возвращаем терминал после того, как программа полностью завершится
