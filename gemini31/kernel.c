...
void main() {
    ...
    // Главный цикл ОС без тормозов (Uncapped FPS)
    while(1) {
        ...
        extern int gui_active;
                ...
                // --- РИСУЕМ НАСТОЯЩИЙ КУРСОР МЫШИ (СТРЕЛКА) ---
                // Левая черная грань
                for(int i=0; i<12; i++) put_pixel(mouse_x, mouse_y + i, VGA_BLACK);
                // Правая диагональ
                for(int i=0; i<8; i++) put_pixel(mouse_x + i, mouse_y + i, VGA_BLACK);
                // Низ стрелки
                for(int i=0; i<5; i++) put_pixel(mouse_x + i, mouse_y + 11, VGA_BLACK);
                for(int i=5; i<8; i++) put_pixel(mouse_x + i, mouse_y + 16 - i, VGA_BLACK);
                
                // Белая заливка внутри
                for(int i=1; i<11; i++) put_pixel(mouse_x + 1, mouse_y + i, VGA_WHITE);
                for(int i=2; i<10; i++) put_pixel(mouse_x + 2, mouse_y + i, VGA_WHITE);
                for(int i=3; i<9; i++) put_pixel(mouse_x + 3, mouse_y + i, VGA_WHITE);
                for(int i=4; i<8; i++) put_pixel(mouse_x + 4, mouse_y + i, VGA_WHITE);
                for(int i=5; i<7; i++) put_pixel(mouse_x + 5, mouse_y + i, VGA_WHITE);
                put_pixel(mouse_x + 6, mouse_y + 6, VGA_WHITE);
                // ------------------------------------------------
                
                swap_buffers();
            
            // Благодаря rep movsd в графическом модуле это происходит моментально!
            swap_buffers();
            
        } else {
            // Восстановление терминала после закрытия оконного режима
            if (was_gui_active) {
            ...
            // В терминале мышь рисует себя сама
