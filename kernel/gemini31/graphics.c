...
// Наш невидимый буфер остается 32-битным (так удобнее работать процессору)
...
// ==========================================
// ИДЕАЛЬНАЯ СИНХРОНИЗАЦИЯ (32-bit -> 24-bit)
// ==========================================
void swap_buffers() {
    // ВАЖНО: Видеокарта в режиме 0x115 понимает только 24-bit (3 байта на пиксель).
    // Поэтому мы создаем локальный указатель типа char (1 байт) на память видеокарты.
    unsigned char* vga_bytes = (unsigned char*) vbe_framebuffer;
    
    int vga_idx = 0;
    
    for (int i = 0; i < screen_w * screen_h; i++) {
        unsigned int color = backbuffer[i]; // Берем 32-битный цвет из буфера
        
        // Разбираем его на 3 байта и пишем в видеокарту (Blue, Green, Red)
        vga_bytes[vga_idx++] = color & 0xFF;         // Синий
        vga_bytes[vga_idx++] = (color >> 8) & 0xFF;  // Зеленый
        vga_bytes[vga_idx++] = (color >> 16) & 0xFF; // Красный
    }
}
// ==========================================
...
// ==========================================
// ХАРДВАРНЫЙ КУРСОР ДЛЯ ТЕРМИНАЛА
// ==========================================

// Рисует курсор мыши НАПРЯМУЮ в видеокарту, не пачкая Backbuffer
void draw_mouse_cli(int x, int y) {
    unsigned char* vga_bytes = (unsigned char*) vbe_framebuffer;
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            if (x+j >= screen_w || y+i >= screen_h) continue;
            int idx = ((y + i) * screen_w + (x + j)) * 3;
            vga_bytes[idx] = 0xFF;     // B
            vga_bytes[idx+1] = 0xFF;   // G
            vga_bytes[idx+2] = 0xFF;   // R
        }
    }
}

// Восстанавливает пиксели из чистого Backbuffer'а напрямую в видеокарту (Стирает мышь)
void erase_mouse_cli(int x, int y) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            vga_bytes[idx] = color & 0xFF;
            vga_bytes[idx+1] = (color >> 8) & 0xFF;
            vga_bytes[idx+2] = (color >> 16) & 0xFF;
        }
    }
}