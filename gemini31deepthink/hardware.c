...
void init_gdt(void) {
    ...
    gdt_set_gate(3, 0, 0xFFFFFFFF, 0xFA, 0xCF); gdt_set_gate(4, 0, 0xFFFFFFFF, 0xF2, 0xCF);
    gdt_set_gate(5, (uint32_t)&tss_entry, sizeof(tss_entry), 0xE9, 0x00);
    fast_fill32(&tss_entry, 0, sizeof(tss_entry)/4); tss_entry.ss0 = 0x10; tss_entry.esp0 = 0x90000; tss_entry.iomap_base = sizeof(tss_entry);
...
void get_rtc_time(int *h, int *m, int *s) {
    ...
    uint8_t sec = read_rtc(0x00); uint8_t min = read_rtc(0x02); uint8_t hr = read_rtc(0x04); uint8_t statusB = read_rtc(0x0B);
    if (!(statusB & 0x04)) { sec = (sec & 0x0F) + ((sec / 16) * 10); min = (min & 0x0F) + ((min / 16) * 10); hr = (hr & 0x0F) + (((hr & 0x70) / 16) * 10); }
    *s = sec; *m = min; *h = (hr + 7) % 24; 
}
...
void keyboard_handler_isr() {
    uint8_t sc = inb(0x60); if (sc == 0xE0) { outb(0x20, 0x20); return; }
    if (!(sc & 0x80)) {
        char c = 0; if (sc == 0x4B) { c = 17; } else if (sc == 0x4D) { c = 18; } else if (sc == 0x48) { c = 19; } else if (sc == 0x50) { c = 20; } else if (sc < 128) { c = keymap[sc]; }
        if (c) { key_buffer = c; key_ready = 1; app_key_buffer = c; app_key_ready = 1; }
    } outb(0x20, 0x20); 
}
...
void init_hardware() {
    extern void init_paging(void); init_paging(); 
    init_gdt(); 
    ...
    set_idt_gate(129, (uint32_t)yield_isr_wrapper, 3); 
