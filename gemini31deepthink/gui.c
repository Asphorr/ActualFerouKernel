...
typedef struct { int active, x, y, w, h; char title[32]; uint32_t* front_canvas; uint32_t* back_canvas; int state[16]; } Window;
...
int gui_is_focused(int id) { if (win_count > 0 && z_order[win_count-1] == id) return 1; return 0; }
...
int gui_register_window(const char* title, int w, int h) {
    for (int i = 0; i < MAX_WINS; i++) {
        if (!wins[i].active) {
            wins[i].active = 1; wins[i].x = 100 + spawn_offset; wins[i].y = 80 + spawn_offset; spawn_offset = (spawn_offset + 30) % 300; 
...
void gui_get_mouse_win(int id, int* mx, int* my, int* clk) {
    if (id < 0 || id >= MAX_WINS || !wins[id].active) { *clk = 0; *mx = -1000; *my = -1000; return; }
    ...
    *mx = gx - wins[id].x - 2; *my = gy - wins[id].y - 26; 
    ...
    for (int i = win_count - 1; i >= 0; i--) {
        ...
        if (gx >= cw->x && gx <= cw->x + cw->w + 4 && gy >= cw->y && gy <= cw->y + cw->h + 30) { top_id = cid; break; }
    }
...
void draw_desktop() {
    ...
    if (my >= screen_h - 40) {
        ...
        for (int i = win_count - 1; i >= 0; i--) {
            ...
            if (mx >= w->x && mx <= w->x + w->w + 4 && my >= w->y && my <= w->y + w->h + 30) { on_desk = 0; break; }
        }
        if (start_menu_open) {
    ...
    for (int i = 0; i < win_count; i++) {
        int id = z_order[i]; Window* w = &wins[id]; int is_active = (i == win_count - 1);
        int tb_hover = (mx >= tb_x && mx <= tb_x + 40 && my >= screen_h - 40);
        draw_alpha_rect(tb_x, screen_h - 40, 40, 40, tb_hover ? 0x555555 : (is_active ? 0x444444 : 0x111111), 180);
    ...
    if (start_menu_open) {
        ...
        while (installed_apps[idx] && list_y < sm_y + sm_h - 60) {
            ...
            if (n >= 11 && name[8] == 'B' && name[9] == 'I' && name[10] == 'N') {
                int item_hover = (mx >= sm_x + 5 && mx <= sm_x + sm_w - 5 && my >= list_y && my <= list_y + 24);
...
void update_gui() {
    ...
    if (click) {
        ...
        if (start_menu_open) {
            ...
            if (mx >= sm_x && mx <= sm_x + sm_w && my >= sm_y && my <= sm_y + sm_h) {
                ...
                while (installed_apps[idx] && list_y < sm_y + sm_h - 45) {
                    ...
                    if (n >= 11 && name[8] == 'B' && name[9] == 'I' && name[10] == 'N') { if (mx >= sm_x + 5 && mx <= sm_x + sm_w - 5 && my >= list_y && my <= list_y + 24) { exec_app(name); start_menu_open = 0; prev_click = mlp; return; } list_y += 30; }
                }
        ...
        if (my >= screen_h - 40) {
            int tb_x = 100;
            for (int i = 0; i < win_count; i++) {
                if (mx >= tb_x && mx <= tb_x + 40) { int id = z_order[i]; for (int j = i; j < win_count - 1; j++) { z_order[j] = z_order[j+1]; } z_order[win_count - 1] = id; return; } tb_x += 44;
            } return;
        }

        for (int i = win_count - 1; i >= 0; i--) {
            int id = z_order[i]; Window* w = &wins[id];
            if (mx >= w->x && mx <= w->x + w->w + 4 && my >= w->y && my <= w->y + w->h + 30) {
                if (i != win_count - 1) { focus_changed_this_click = 1; }
                for (int j = i; j < win_count - 1; j++) { z_order[j] = z_order[j+1]; } z_order[win_count - 1] = id;
                if (mx >= w->x + w->w - 22 && mx <= w->x + w->w - 2 && my >= w->y + 2 && my <= w->y + 26) { w->active = 0; kfree(w->front_canvas); kfree(w->back_canvas); win_count--; }
                else if (my <= w->y + 26) { drag_win = id; drag_off_x = mx - w->x; drag_off_y = my - w->y; } 
                prev_click = mlp; return; 
            }
        }
        
        // ИСПРАВЛЕНИЕ: Вызываем правильные 11-символьные имена FAT16!
        if (start_menu_open == 0 && my < screen_h - 40) {
            if (mx >= 30 && mx <= 80 && my >= 30 && my <= 90) { exec_app("EXPLORERBIN"); } 
            else if (mx >= 30 && mx <= 80 && my >= 110 && my <= 170) { exec_app("SYSINFO BIN"); } 
            else if (mx >= 30 && mx <= 80 && my >= 190 && my <= 250) { exec_app("SETTINGSBIN"); } 
            else if (mx >= 30 && mx <= 80 && my >= 270 && my <= 330) { exec_app("CLOCK   BIN"); }
        }
    }
    if (drag_win != -1) { wins[drag_win].x = mx - drag_off_x; wins[drag_win].y = my - drag_off_y; } prev_click = mlp;
}

void reset_desktop() { for (int i = 0; i < MAX_WINS; i++) { if (wins[i].active) { kfree(wins[i].front_canvas); kfree(wins[i].back_canvas); } wins[i].active = 0; } win_count = 0; drag_win = -1; prev_click = 0; spawn_offset = 0; apps_loaded = 0; start_menu_open = 0; }